  ...
  public void _jspService(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      out.write("        ");
            ...
            try{
            String query = "select pass from users where email like '"+request.getParameter("uname")+"'";
            String retrieved_pass="";
            rs = st.executeQuery(query);
            
            if(rs.next())
                retrieved_pass = rs.getString(0);
            else
                response.sendRedirect("http://localhost:8080/training_jsp/index.html");
            
            if(retrieved_pass.equals(pass))
                response.sendRedirect("http://localhost:8080/training_jsp/details.jsp");
